...
func main() {
	/*
		slice := arr[start:end]
		 ...
		 从已有的数组上，直接创建切片，该切片的底层数组就是当前的数组。修改切片也就是修改这个数组中的数据
		 	长度是从start到end切割的数据量。
			但是容量从start到数组的末尾。
	*/
	...
	fmt.Println("----------4.更改切片的内容,数组/其他切片跟着改--------------------")
	s2[2] = 200
	fmt.Println(a)
	fmt.Println(s1)
	fmt.Println(s2)
	...
	fmt.Println("----------5.添加切片的内容，但不扩容--------------------")
	s1 = append(s1, 1, 1, 1, 1)
	fmt.Println(a)
	fmt.Println(s1)
	fmt.Println(s2)
	fmt.Println(s3)
	fmt.Println("----------6.添加元素直到切片扩容--------------------")
